...
pkgs.writeShellApplication {
  ...
  text = ''
    ...
    # 5. Metis leaf skills, disabled by default.
    ...
    # 6. Git identity, configuration first, host git configuration fallback.
    ...
    # 7. Workspace-derived identifiers, an ephemeral SSH key, and the SSH
    ...
    # 8. Export the environment consumed by the VM build.
    ...
    # 9. Build the target MicroVM.
    VM_PATH="$(nix build "${flakeRef}#aegis-vm-${system}" --impure --no-link --print-out-paths)"

    # 10. Start the virtiofs daemons. macOS uses built-in 9p shares, which
    # need no virtiofsd.
    ...
    # 11. Run the MicroVM in the background.
    VM_LOG="/tmp/aegis-vm-$VM_MOUNT_TAG.log"
    "''${VM_PATH}/bin/microvm-run" "$@" &> "$VM_LOG" &
    VM_PID=$!

    # 12. Wait for the guest SSH server.
