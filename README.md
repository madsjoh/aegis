...
is retained. Override these base directories with `XDG_DATA_HOME` and
`XDG_CACHE_HOME`.
...
The state directory holds the configuration snapshot, an `opencode/` directory,
and a `run/` directory. The `opencode/` subdirectories (`state` and `share`) are
mounted into the guest under `.local/state/opencode` and `.local/share/opencode`
