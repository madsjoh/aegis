...
directory, and a `run/` directory. The `opencode/` subdirectories (`state` and
`share`) are mounted into the guest under `.local/state/opencode` and
`.local/share/opencode` so sessions and data persist between runs. The OpenCode
configuration under `.config/opencode` is not persisted; it is generated by
Metis on every boot. The `run/` directory holds the lock, the virtiofsd sockets,
and the logs. The share directory holds the persisted SSH key. Override the base
directories with `XDG_STATE_HOME` and `XDG_DATA_HOME`.
